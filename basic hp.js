var artName_array = ["Guernica","View of the Asylum and Chapel of Saint-Rémy","A Wheatfeil with Cypresses","Starry Night","Olive Orchard","Self Potrait","Self Potrait","Self Potrait","Self Potrait","Fictional Character","The persistence of Memory","Old Guitarist","Sunday Afternoon on the Island of La Grande Jatte","Girl with a Pearl Earring","The Great Wave off Kanagawa","Realism","The Lighthouse","The Snake Artist","Impression Sunrise","The Stanza della segnatura","Self Potrait","The parralel world"];
var artistName_array = ["PABLO PICASSO","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","VINCENT VAN GOGH","Unknown ARTIST","SALVADOR DALI","PABLO PICASSO","Georges Seurat","Johannes Vermeer","Katsushika Hokusai","Gustave Courbet","Unknown Artist","Unknown Artist","Claude Monet","Raphael","Rembrandt van Rijn","HC"];
...
window.onscroll=function()
{
  ...
  function modal_img(value)                                                             //image 1 clicked
    ...
    function shownew()
    {
        var x = (localStorage.getItem("path"));
...
function find()
{
    var img_bk = document.getElementsByClassName('srch_img_cont');
    for(let i=0; i<img_bk.length; i++)
    {
      img_bk[i].style.display = 'none';
    }
    document.getElementById("resultdc").style.display = 'none';
    //document.getElementById("result").innerHTML = "";
    var k = 0;
    var sw = document.getElementById("search_form").searchbar.value;
    sw = sw.toLowerCase();
    for(let i = 0; i<artName_array.length; i++)
    {
        var count = 0;
        var txt = artName_array[i];
        txt = txt.toLowerCase();
        var artist = artistName_array[i];
        artist = artist.toLowerCase();
        var length = Math.max(txt.length, artist.length);
        for(let j = 0; j<length; j++)
        {
            if(((txt.charAt(j) == sw.charAt(0)) || (artist.charAt(j) == sw.charAt(0))) && (count == 0))
            {
                var word = create_word(j, sw.length, txt);
                var wordn = create_word(j, sw.length, artist);
                if((word == sw) || (wordn == sw))
                {
                    count = 1;
                   // document.getElementById("result").innerHTML = document.getElementById("result").innerHTML + "<br>" + artName_array[i];
                    var st_name = "s" + k;
                    //document.getElementById("result").innerHTML = st_name;
                    localStorage.setItem(st_name, i);
                    var ur = "url(p" + i + ".jpg)";
                    img_bk[k].style.display = 'block';
                    img_bk[k].style.backgroundImage = ur;
                    k++;
                }
            }
        } 
    ...
    if(k==0)
    {
      document.getElementById("resultdc").style.display = 'block';
    }
}

function create_word(st, len, wrd)
{
    var v1 = st + len - 1;
    var new_wrd = "";
    if(v1 < wrd.length)
    {
        for(let i=st; i<(v1+1); i++)
        {
            new_wrd = new_wrd + wrd.charAt(i);
        }
        return new_wrd;
    }
    else
